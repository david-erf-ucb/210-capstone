...
def query_culture_mimic(client, subject_to_keep):
    # =============================================================================
    # SKIPPED: Culture data extraction (MIMIC-IV 3.1 schema change)
    # =============================================================================
    # The `mimiciv_derived.culture` table no longer exists in MIMIC-IV 3.1.
    # 
    # FUNCTIONALITY LOST:
    #   - specimen_culture: Culture site information (14 categorical features)
    #   - positive_culture: Whether culture was positive for pathogens
    #   - has_sensitivity: Whether antibiotic sensitivity testing was performed
    #   - screen: Whether this was a screening culture
    #
    # POTENTIAL FIX: Rewrite query using `mimiciv_3_1_hosp.microbiologyevents`:
    #   - specimen -> spec_type_desc
    #   - positive_culture -> org_name IS NOT NULL AND org_name != 'NEGATIVE'
    #   - has_sensitivity -> interpretation IS NOT NULL
    #   - screen -> derive from test_name containing 'SCREEN'
    #
    # See SCHEMA_MIGRATION_NOTES.md for full details.
    # =============================================================================
    import pandas as pd
    print("WARNING: Culture data SKIPPED - mimiciv_derived.culture table not available in v3.1")
    culture = pd.DataFrame(columns=[
        'subject_id', 'charttime', 'specimen', 'screen', 
        'positive_culture', 'has_sensitivity', 'hadm_id', 'stay_id', 'icu_intime'
    ])
    return culture
...
def query_heparin_mimic(client, subject_to_keep):
    # =============================================================================
    # SKIPPED: Heparin data extraction (MIMIC-IV 3.1 schema change)
    # =============================================================================
    # The `mimiciv_derived.heparin` table no longer exists in MIMIC-IV 3.1.
    #
    # FUNCTIONALITY LOST:
    #   - heparin: Binary indicator of heparin administration during ICU stay
    #
    # POTENTIAL FIX: Rewrite query using `mimiciv_3_1_icu.inputevents`:
    #   SELECT ie.stay_id, ie.starttime, ie.endtime, i.subject_id, i.hadm_id, 
    ...
    #   WHERE ie.itemid IN (225152, 225975, 229597, 230044)  -- Heparin itemids
    #     AND ie.amount > 0
    ...
    # =============================================================================
    import pandas as pd
    print("WARNING: Heparin data SKIPPED - mimiciv_derived.heparin table not available in v3.1")
    heparin = pd.DataFrame(columns=[
        'subject_id', 'starttime', 'endtime', 'hadm_id', 'stay_id', 'icu_intime', 'icu_outtime'
    ])
    return heparin
